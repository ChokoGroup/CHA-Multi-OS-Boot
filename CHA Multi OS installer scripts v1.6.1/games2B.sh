...
if [ -f "/.choko/Run Batocera 720p.nfo" ]
then
  rm "/.choko/Run Batocera 720p."*
  echo "Menu option \"Run Batocera (720p)\" deleted (Batocera system files and partition were not deleted)."
else
  echo "\"/.choko/Run Batocera 720p.*\" not found."
fi
sleep 3
